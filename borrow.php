...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'borrow') {
    ...
    if (empty($errors)) {
        ...
        $t_check = mysqli_real_escape_string($conn, $book_title);
        $stock_result = mysqli_query($conn, "SELECT shelf_stock FROM shelf_books WHERE shelf_title = '$t_check' LIMIT 1");
    ...
    if (empty($errors)) {
        ...
        if (mysqli_query($conn, $sql)) {
            $t_escaped = mysqli_real_escape_string($conn, $book_title);
            mysqli_query($conn, "UPDATE shelf_books SET shelf_stock = shelf_stock - 1 WHERE shelf_title = '$t_escaped' AND shelf_stock > 0");
...
// FIX 1: Define $genreEmoji BEFORE it is used anywhere
$genreEmoji = [
    'Fiction'        => '📖',
    'Non-Fiction'    => '📰',
    'Science Fiction'=> '🚀',
...
// Collect unique genres for tabs
...
<style>
  :root {
    ...
    --primary:  #4f8ef7;
    --primary-glow: rgba(79,142,247,0.18);
    --green:    #34d399;
    --gold:     #fbbf24;
    --red:      #f87171;
    --text:     #f0f4ff;
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .search-wrap input {
    ...
    border-radius: 50px; color: var(--text); font-size: 14.5px; font-family: inherit;
  ...
  .search-clear {
    ...
    cursor: pointer; display: none; padding: 4px; line-height: 1;
  }

  /* GENRE TABS */
  .genre-tabs {
    display: flex; gap: 7px; flex-wrap: wrap; justify-content: center; margin-bottom: 28px;
  }
  .genre-tab {
  ...
  .book-card:hover {
    ...
    box-shadow: 0 20px 50px rgba(0,0,0,.55), 0 0 0 1px var(--primary);
  ...
  .cover-placeholder {
    ...
    gap: 10px; background: linear-gradient(160deg, var(--card2), var(--bg));
  }
  .cp-emoji { font-size: 50px; }
  .cp-genre {
    font-size: 9.5px; font-weight: 700; letter-spacing: 1.2px; text-transform: uppercase;
    padding: 3px 10px; border-radius: 20px;
    background: var(--primary-glow); color: var(--primary); border: 1px solid rgba(79,142,247,.3);
  }

  .borrow-hint {
    position: absolute; inset: 0;
    background: linear-gradient(to top, rgba(79,142,247,.85) 0%, transparent 55%);
  ...
  .book-meta .bm-title {
    font-size: 13.5px; font-weight: 700; line-height: 1.35; margin-bottom: 3px; color: var(--text);
  ...
  .modal-steps {
    ...
    padding: 0 24px; flex-shrink: 0; background: var(--surface);
  }
  .step-tab {
    padding: 13px 0; margin-right: 24px; font-size: 12px; font-weight: 600;
  ...
  .step-num {
    ...
    background: var(--subtle); color: var(--muted); flex-shrink: 0;
  }
  ...
  .field-group input.is-valid { border-color: var(--green); }
  ...
  .drv-days {
    background: var(--primary-glow); border-radius: 20px;
    padding: 2px 10px; font-size: 11.5px; font-weight: 700; color: var(--primary);
  ...
  .carousel-item.featured {
    flex: 0 0 170px; height: 240px;
    border: 2px solid var(--primary);
    box-shadow: 0 0 30px rgba(79,142,247,.25);
  ...
  .carousel-placeholder {
    ...
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    gap: 8px; background: linear-gradient(160deg, var(--card2), var(--bg));
  }
  .carousel-overlay {
    position: absolute; inset: 0;
  ...
  .carousel-btn {
    ...
    display: flex; align-items: center; justify-content: center;
    z-index: 10; transition: all .2s; backdrop-filter: blur(6px);
  }
...
<div class="container">
  ...
  <?php else: ?>
      ...
      <div class="carousel-wrap">
        ...
        <div class="carousel-track-outer">
          <div class="carousel-track" id="carouselTrack">
            <?php foreach ($shelf_books as $idx => $sb):
    ...
    <!-- GENRE TABS -->
    <!-- FIX 2: Genre tabs now correctly rendered with PHP loop -->
      ...
      <div class="book-card"
        ...
        <div class="book-meta">
          ...
          <div style="display:flex; align-items:center; gap:6px; flex-wrap:wrap; margin-top:6px;">
            ...
            <?php if ($stock > 0): ?>
              <span style="font-size:10.5px; font-weight:700; padding:2px 9px; border-radius:20px;
    ...
    <div class="modal-body">
      ...
      <div class="step-panel active" id="step0">
        ...
        <div style="background:var(--card); border:1px solid var(--border); border-radius:10px;
                    padding:16px; margin-bottom:16px; font-size:14px; color:var(--muted); line-height:1.7;"
             id="descBox">
        ...
        <div style="display:flex; align-items:center; gap:10px; background:var(--card);
          ...
          <div>
            <div style="font-size:11px; text-transform:uppercase; letter-spacing:.7px;
                        color:var(--muted); font-weight:700; margin-bottom:3px;">Availability</div>
      ...
      <form method="POST" action="borrow.php" id="borrowForm">
        ...
        <div class="step-panel" id="step2">
          ...
          <div class="date-range-viz">
            <div class="drv-item">
              <div class="drv-label">Borrow Date</div>
              <div class="drv-date" id="vizBorrowDate">—</div>
              <div class="drv-sub">Today</div>
            </div>
            <div class="drv-sep">→</div>
            <div class="drv-item">
              <div class="drv-label">Due Date</div>
              <div class="drv-date" id="vizDueDate">—</div>
              <div class="drv-days" id="vizDays">7 days</div>
            </div>
          </div>
          <div class="field-row">
            <div class="field-group">
          ...
          <div class="summary-card">
            ...
            <div class="sc-body">
              ...
              <div class="sc-row">
                ...
                <div>
                  <div class="sc-val" id="sumDue">—</div>
                  <span class="sc-badge" id="sumDays">7 days</span>
                </div>
              </div>
            </div>
          </div>
...
function openBorrowModal(card) {
  ...
  if (currentBookData.cover) {
    thumb.innerHTML = `<img src="${currentBookData.cover}" alt="" onerror="this.parentNode.textContent='${currentBookData.emoji}'">`;
  } else {
    thumb.textContent = currentBookData.emoji;
  }
  ...
  descBox.innerHTML = currentBookData.desc
    ? currentBookData.desc
  ...
  if (currentBookData.stock > 0) {
    stockEl.textContent = `✓ ${currentBookData.stock} ${currentBookData.stock === 1 ? 'copy' : 'copies'} available`;
  ...
  document.getElementById('sBlock').value  = '';
...
function goToStep(n) {
  ...
  if (n === 3) {
    if (currentBookData.stock <= 0) {
      ...
      btnNext.textContent = '✕ Out of Stock';
      btnNext.style.background  = 'var(--red)';
      btnNext.style.opacity     = '0.6';
      btnNext.style.cursor      = 'not-allowed';
      btnNext.className         = 'btn';
    } else {
      btnNext.disabled    = false;
      btnNext.textContent = 'Confirm Borrow ✓';
    ...
    btnNext.disabled    = false;
    btnNext.textContent = 'View Borrow Form →';
    btnNext.style.background  = '';
    btnNext.style.opacity     = '';
    btnNext.style.cursor      = '';
...
function validateStep(n) {
  ...
  if (n === 1) {
    ...
    if (!document.getElementById('sNumber').value.trim()) errs.push('Student number is required.');
  } else if (n === 2) {
    const b = document.getElementById('dBorrow').value;
    const d = document.getElementById('dDue').value;
...
function nextStep() {
  if (currentStep === 3) {
    document.getElementById('fBorrowed').value      = document.getElementById('dBorrow').value;
    document.getElementById('fDue').value           = document.getElementById('dDue').value;
    document.getElementById('fStudentName').value   = document.getElementById('sName').value.trim();
    document.getElementById('fStudentBlock').value  = document.getElementById('sBlock').value.trim();
    document.getElementById('fStudentNumber').value = document.getElementById('sNumber').value.trim();
...
function populateSummary() {
  ...
  const bd = document.getElementById('dBorrow').value;
  const dd = document.getElementById('dDue').value;
...
function updateDateViz() {
  ...
  if (bd && dd) {
    const days = Math.round((new Date(dd) - new Date(bd)) / 86400000);
    document.getElementById('vizDays').textContent = days > 0 ? `${days} days` : '⚠ Invalid';
    document.getElementById('vizDays').style.color = days > 0 ? '' : 'var(--red)';
  }
}
...
function filterByGenre(genre, tab) {
  ...
  applyFilters(document.getElementById('searchInput').value.toLowerCase().trim(), genre);
}
function applyFilters(q, genre) {
  const cards = document.querySelectorAll('#booksGrid .book-card');
  let visible = 0;
  cards.forEach(c => {
    const gm = genre === 'all' || c.dataset.genre === genre;
