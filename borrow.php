...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .btn { display:inline-flex; align-items:center; gap:7px; padding:9px 18px; border:none; border-radius:9px; font-size:13px; font-weight:700; font-family:inherit; cursor:pointer; text-decoration:none; transition:all .2s; white-space:nowrap; }
  ...
  .cp-genre { font-size:9.5px; font-weight:700; letter-spacing:1.2px; text-transform:uppercase; padding:3px 10px; border-radius:20px; background:var(--primary-light); color:var(--primary); border:1px solid rgba(59,125,216,.3); }
  .borrow-hint { position:absolute; inset:0; background:linear-gradient(to top,rgba(59,125,216,.8) 0%,transparent 55%); display:flex; align-items:flex-end; justify-content:center; padding-bottom:12px; opacity:0; transition:opacity .25s; }
  ...
  .carousel-item { flex:0 0 140px; cursor:pointer; border-radius:var(--radius-lg); overflow:visible; position:relative; height:210px; border:1.5px solid var(--border); transition:transform .25s,box-shadow .25s; background:var(--surface); }
  .carousel-item:hover { transform:translateY(-10px) scale(1.05); box-shadow:var(--shadow-md); z-index:10; }
  .carousel-item.featured { flex:0 0 170px; height:240px; border:1.5px solid rgba(217,119,6,.4); box-shadow:0 0 20px rgba(217,119,6,.12); }
  ...
  .carousel-overlay { position:absolute; inset:0; border-radius:var(--radius-lg); background:linear-gradient(to top,rgba(26,35,64,.85) 0%,transparent 55%); display:flex; flex-direction:column; justify-content:flex-end; padding:12px 10px; pointer-events:none; }
  ...
  .modal { background:var(--surface); border:1.5px solid var(--border); border-radius:20px; width:100%; max-width:540px; box-shadow:0 24px 60px rgba(26,35,64,.18); animation:slideUp .28s cubic-bezier(.34,1.1,.64,1); max-height:95vh; display:flex; flex-direction:column; overflow:hidden; }
  .modal-book-banner { position:relative; padding:22px 24px 18px; background:var(--card2); border-bottom:1.5px solid var(--border); display:flex; gap:16px; align-items:flex-start; flex-shrink:0; }
  .mbb-thumb  { width:56px; height:72px; border-radius:8px; overflow:hidden; background:var(--border); flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:26px; border:1.5px solid var(--border); }
  .mbb-thumb img { width:100%; height:100%; object-fit:cover; }
  .mbb-genre  { font-size:10px; font-weight:800; letter-spacing:.8px; text-transform:uppercase; color:var(--primary); margin-bottom:5px; }
  .mbb-title  { font-family:'Playfair Display',serif; font-size:17px; font-weight:700; line-height:1.3; color:var(--text); margin-bottom:3px; }
  ...
  .step-num { width:20px; height:20px; border-radius:50%; font-size:11px; font-weight:700; display:flex; align-items:center; justify-content:center; background:var(--card2); color:var(--muted); flex-shrink:0; }
  ...
  .field-group input,.field-group select { width:100%; padding:11px 14px; background:var(--card); border:1.5px solid var(--border); border-radius:9px; color:var(--text); font-size:14px; font-family:inherit; outline:none; transition:border-color .2s,box-shadow .2s; }
  .field-group input::placeholder { color:var(--muted); opacity:.7; }
  .field-group input:focus,.field-group select:focus { border-color:var(--primary); box-shadow:0 0 0 3px var(--primary-glow); }
  ...
  .sc-row    { display:flex; justify-content:space-between; align-items:flex-start; gap:10px; padding:7px 0; border-bottom:1px solid var(--border-light); }
  .sc-row:last-child { border-bottom:none; }
  .sc-key    { font-size:12px; color:var(--muted); flex:none; width:110px; font-weight:600; }
            ...
            <div class="carousel-item <?= $isFeatured ? 'featured' : '' ?>"
              ...
              <?php if ($isNew): ?>
                <div style="position:absolute;top:8px;left:8px;z-index:5;
                            ...
                            box-shadow:0 2px 8px rgba(14,168,106,.35);">
                  NEW
                </div>
              <?php endif; ?>
              ...
              <div class="carousel-overlay">
                ...
                <div style="margin-top:6px;font-size:10px;font-weight:700;
                            background:<?= $isFeatured ? 'rgba(217,119,6,.25)' : 'rgba(255,255,255,.12)' ?>;
      ...
      <div class="book-card"
        ...
        <div class="book-meta">
          ...
          <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:6px;">
            ...
            <?php else: ?>
              <span style="font-size:10.5px;font-weight:700;padding:2px 9px;border-radius:20px;background:var(--red-light);color:var(--red);border:1px solid rgba(224,60,60,.2);">
                Out of Stock
              </span>
            <?php endif; ?>
          </div>
  ...
  <div class="modal" role="dialog" aria-modal="true">
    ...
    <div class="modal-body">
      ...
      <div class="step-panel active" id="step0">
        ...
        <div class="about-desc-box" id="descBox"><em style="color:var(--muted)">No description available.</em></div>
        <div class="about-stock-box">
          <span style="font-size:20px;"></span>
          <div>
            <div class="asl">Availability</div>
...
<div class="site-footer">
  Built with the help of <a href="https://claude.ai" target="_blank">Claude.ai</a> — AI assistant by Anthropic
</div>
